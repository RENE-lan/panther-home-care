...
class User(AbstractUser):
    ...
    push_token = models.CharField(max_length=200, blank=True)  # Expo push token
...
class MobileToken(models.Model):
    """Auth token for the React Native app."""
    key = models.CharField(max_length=48, primary_key=True)
    user = models.ForeignKey("accounts.User", related_name="mobile_tokens", on_delete=models.CASCADE)
...
class AccessMatrix(models.Model):
    """Admin-editable role → module permission matrix (singleton). data = {role: {area: bool}}."""
