...
class Client(models.Model):
    ...
    family_code = models.CharField(max_length=8, blank=True, db_index=True)
...
class Invoice(models.Model):
    ...
    description = models.CharField(max_length=200, blank=True, help_text="Description du service")
    ...
    @property
    def remaining(self):
        return max(self.amount - (self.amount_paid or 0), 0)

    @property
    def is_partial(self):
        return 0 < (self.amount_paid or 0) < self.amount

    def apply_payment(self, amt):
        from datetime import date
        self.amount_paid = (self.amount_paid or 0) + amt
        ...
        self.save(update_fields=["amount_paid", "status", "paid_on"])
...
class Message(models.Model):
    ...
    TOPICS = [("CARE", "Soins / Coordinateur"), ("FINANCE", "Finance / Facturation")]
    topic = models.CharField(max_length=10, choices=TOPICS, default="CARE", db_index=True)
...
class Lead(models.Model):
    ...
    notes = models.TextField(blank=True)
    next_action = models.CharField(max_length=200, blank=True, help_text="Prochaine action")
...
class ConsentRecord(models.Model):
    """POPIA/RGPD-style consent ledger for a client's data & care."""
    PURPOSES = [
        ("CARE", "Prestation des soins"),
        ("DATA", "Traitement des données personnelles"),
        ("SHARE_FAMILY", "Partage avec la famille"),
        ("MEDICAL", "Accès aux informations médicales"),
    ]
    client = models.ForeignKey("clients.Client", on_delete=models.CASCADE, related_name="consents")
    purpose = models.CharField(max_length=20, choices=PURPOSES)
    text_version = models.CharField(max_length=20, default="v1")
    granted = models.BooleanField(default=False)
    granted_at = models.DateTimeField(null=True, blank=True)
    withdrawn_at = models.DateTimeField(null=True, blank=True)
    signed_by = models.CharField(max_length=120, blank=True)
    signature = models.TextField(blank=True)  # data-URL / drawn path
...
class DataAccessLog(models.Model):
    """Who looked at a client's sensitive record, and when. Shown to the family."""
    client = models.ForeignKey("clients.Client", on_delete=models.CASCADE, related_name="access_logs")
    ...
    what = models.CharField(max_length=160, default="Consultation du dossier")
...
class LeadEvent(models.Model):
    """Prospect contact history (calls, messages, emails, meetings, notes)."""
    KIND = [("CALL", "Appel"), ("MESSAGE", "Message"), ("EMAIL", "E-mail"),
            ("MEETING", "Réunion"), ("NOTE", "Note"), ("STAGE", "Changement d'étape")]
    lead = models.ForeignKey("clients.Lead", on_delete=models.CASCADE, related_name="events")
    kind = models.CharField(max_length=10, choices=KIND, default="NOTE")
