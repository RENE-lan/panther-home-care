...
# Load a local ".env" file (simple KEY=value lines) if present, so setting the
# Claude key is as easy as writing AI_API_KEY=sk-ant-... in a file next to manage.py.
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "accounts.context_processors.agency",
                    "accounts.context_processors.social_enabled",
                    "accounts.context_processors.perms",
            ],
        },
    },
]
...
# conn_max_age keeps DB connections alive across requests — critical when the
# database is remote (Render/managed PG): it removes a full connect round-trip
# on every request, which is often the biggest source of "slow login/pages".
_CONN_MAX_AGE = int(os.environ.get("DB_CONN_MAX_AGE", "600"))
if os.environ.get("DATABASE_URL", "").startswith("postgres"):
    import dj_database_url  # optional dependency
    DATABASES = {"default": dj_database_url.parse(
        os.environ["DATABASE_URL"], conn_max_age=_CONN_MAX_AGE, conn_health_checks=True)}
...
AI_PROVIDER = os.environ.get("AI_PROVIDER", "anthropic").lower()  # openai | anthropic | gemini
...
# The mobile app hits /api/mobile/ from the device; allow it broadly.
...
# Long-cache hashed static files so a CDN (Cloudflare) can serve them from the edge.
