...
def _daily_brief_impl(now=None):
    """
...
# ============================================================================
def _coordinator_actions_impl(limit=6):
...
def daily_brief(*args, **kwargs):
    ...
    v = cache.get(key)
    if v is None:
        v = _daily_brief_impl(*args, **kwargs)
        cache.set(key, v, 45)  # 45s — dashboard freshness without recomputing every load
    return v


def coordinator_actions(limit=6):
    key = f"ai:coordinator_actions:{limit}"
    v = cache.get(key)
    if v is None:
        v = _coordinator_actions_impl(limit)
        cache.set(key, v, 45)
    return v
