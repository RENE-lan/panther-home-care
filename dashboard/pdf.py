...
def invoice_pdf(invoice):
    ...
    _table(doc, ["Description", "Heures", "Tarif", "Montant"],
           [[invoice.description or f"Services de soins — {invoice.period}", f"{hours:g}",
    ...
    remaining = max(total - paid_amt, 0)
    if paid_amt > 0:
        doc.set_font("Helvetica", "", 10)
        doc.set_text_color(20, 120, 60)
        doc.cell(150, 6, _s("Payé"), align="R")
        doc.cell(38, 6, _s(f"{paid_amt:.2f} {cur}"), align="R", new_x="LMARGIN", new_y="NEXT")
        if remaining > 0:
            doc.set_text_color(190, 40, 40)
            doc.cell(150, 6, _s("Reste à payer"), align="R")
            doc.cell(38, 6, _s(f"{remaining:.2f} {cur}"), align="R", new_x="LMARGIN", new_y="NEXT")
    ...
    if paid:
        ...
        doc.set_text_color(154, 106, 16)
        label = f"PARTIELLEMENT PAYÉE — reste {remaining:.2f} {cur}"
    else:
        doc.set_text_color(190, 40, 40)
