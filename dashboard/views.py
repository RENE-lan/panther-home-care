...
def home(request):
    ...
    all_inv = list(Invoice.objects.all())
    billing = {
        "billed": sum(i.amount for i in all_inv),
        "revenue_month": sum(i.amount for i in all_inv if i.issued_date >= month_start.date()),
        "paid": sum((i.amount_paid or 0) for i in all_inv),
        "outstanding": sum(i.remaining for i in all_inv),
        "overdue": sum(i.remaining for i in all_inv
                       if i.remaining > 0 and (i.due_date or i.issued_date) < today),
    }
    # 6-month finance chart (Facturé vs Encaissé) for the dashboard
    fin_labels, fin_billed, fin_paid = [], [], []
    _d = today.replace(day=1)
    _seq = []
    for _ in range(6):
        _seq.append(_d)
        _d = (_d - timezone.timedelta(days=1)).replace(day=1)
    for _m in reversed(_seq):
        _me = (_m + timezone.timedelta(days=32)).replace(day=1)
        _mi = [i for i in all_inv if _m <= i.issued_date < _me]
        fin_labels.append(_m.strftime("%b %y"))
        fin_billed.append(round(float(sum(i.amount for i in _mi))))
        fin_paid.append(round(float(sum((i.amount_paid or 0) for i in _mi))))
    finance_chart = {"labels": fin_labels, "billed": fin_billed, "paid": fin_paid}
    tasks = {
    ...
    # ---- Operations Command Center: today's ops + priority summary ----
    _now = timezone.localtime()
    checkedin = sum(1 for v in todays if getattr(v, "check_in_at", None))
    at_risk = sum(1 for v in todays if (not v.is_uncovered) and v.status == VisitStatus.SCHEDULED
                  and v.scheduled_start < _now and not getattr(v, "check_in_at", None))
    today_ops = {"visits": total, "covered": confirmed, "uncovered": len(uncovered),
                 "at_risk": at_risk, "evv": checkedin, "late": len(late), "incidents": inc["total"]}
    from ai.attention import attention_items as _attn_items
    _attention = len(_attn_items(50)) + inc["medium"] + (certs_expired if certs_expired else 0)
    _urgent = len(uncovered) + inc["critical"] + inc["high"] + len(late)
    _ok = completed + max(confirmed - len(late), 0)
    priority = {"urgent": _urgent, "attention": _attention, "ok": _ok}
    context = {
        "today_ops": today_ops,
        "priority": priority,
        "priorities": (__import__("ai.priorities", fromlist=["next_actions"]).next_actions(6) if _office_only(request) else []),
        "ai_groups": (lambda _a: {
            "high": [x for x in _a if x["urgency"] == "high"],
            "medium": [x for x in _a if x["urgency"] == "medium"],
            "low": [x for x in _a if x["urgency"] == "low"],
            "count": len(_a),
        })(__import__("ai.priorities", fromlist=["next_actions"]).next_actions(12)) if _office_only(request) else None,
        ...
        "attention": (__import__("ai.attention", fromlist=["attention_items"]).attention_items(6) if _office_only(request) else []),
        "tasks": tasks,
        "command": command,
...
def approve_assignment(request, pk):
    ...
    if caregiver:
        ...
        messages.success(request, f"{caregiver.code} affecté à {visit.client.code}.")
...
def client_360(request, pk):
    ...
    from clients.privacy import log_access, set_consent
    client = get_object_or_404(Client, pk=pk)
    if _office_only(request):
        log_access(request.user, client)
    if request.method == "POST" and _office_only(request) and request.POST.get("consent_purpose"):
        set_consent(client, request.POST["consent_purpose"],
                    request.POST.get("consent_action") == "grant", actor=request.user)
        messages.success(request, "Consentement mis à jour.")
        return redirect("dashboard:client_360", pk=client.pk)
    ...
    from clients.privacy import consent_status
    from ai.timeline import client_timeline
    from ai.attention import attention_for_client
    from incidents.models import Incident
    from collab.models import Meeting
    _cg_ids = list(client.visits.filter(caregiver__isnull=False).order_by("-scheduled_start")
                   .values_list("caregiver_id", flat=True)[:20])
    _seen = []
    for cid in _cg_ids:
        if cid not in _seen:
            _seen.append(cid)
    from caregivers.models import Caregiver as _CG
    care_team = list(_CG.objects.filter(id__in=_seen[:4]))
    open_incidents = list(Incident.objects.filter(client=client).exclude(status="RESOLVED").order_by("-created_at")[:4])
    upcoming_meetings = list(Meeting.objects.filter(client=client).exclude(status__in=["DONE", "CANCELLED", "DECLINED"]).order_by("proposed_at")[:4])
    from clients.models import Invoice as _Inv
    balance = sum(i.remaining for i in _Inv.objects.filter(client=client))
    return render(request, "dashboard/client_360.html", {
        "client": client,
        "journey": client_timeline(client),
        "lifecycle": __import__("ai.journey", fromlist=["client_journey"]).client_journey(client),
        "attention": attention_for_client(client),
        "care_team": care_team,
        "open_incidents": open_incidents,
        "upcoming_meetings": upcoming_meetings,
        "balance": balance,
        "consent_rows": consent_status(client),
        "access_log": client.access_logs.all()[:8],
...
def family_messages(request):
    ...
    if client is None:
        return redirect("dashboard:home")
    channel = request.GET.get("c", "CARE")
    if channel not in ("CARE", "FINANCE"):
        channel = "CARE"
    if request.method == "POST":
        body = request.POST.get("body", "").strip()
        topic = request.POST.get("topic", "CARE")
        topic = topic if topic in ("CARE", "FINANCE") else "CARE"
        if body:
            Message.objects.create(client=client, sender=request.user,
                                   from_agency=False, topic=topic, body=body)
            try:
                from accounts.models import User, Role
                from notifications.services import notify
                roles = [Role.FINANCE] if topic == "FINANCE" else [Role.COORDINATOR, Role.MANAGER]
                for u in User.objects.filter(role__in=roles)[:20]:
                    notify(u, f"Message famille — {client.code}", body[:120])
            except Exception:
                pass
        return redirect(f"{request.path}?c={topic}")
    client.messages.filter(from_agency=True, read=False).update(read=True)
    care = client.messages.filter(topic="CARE").select_related("sender")
    finance = client.messages.filter(topic="FINANCE").select_related("sender")
    return render(request, "dashboard/family_messages.html",
                  {"client": client, "care": care, "finance": finance, "channel": channel})
...
def care_insights(request):
    ...
    if request.user.is_authenticated and not can(request.user, "care_insights"):
        return redirect("dashboard:home")
    ...
    ctx["map_points"] = [
        {"lat": float(c.latitude), "lng": float(c.longitude), "code": c.code,
         "name": c.full_name, "risk": c.risk_level}
        for c in Client.objects.filter(active=True)
        if getattr(c, "latitude", None) and getattr(c, "longitude", None)]
...
def billing(request):
    ...
    kpis = {
        "billed": sum(i.amount for i in all_inv),
        "due": sum(i.remaining for i in all_inv),
        "paid": sum((i.amount_paid or 0) for i in all_inv),
    }
    # Finance charts: billed vs collected over the last 6 months
    fin_labels, fin_billed, fin_paid = [], [], []
    ...
    for m in reversed(seq):
        m_end = (m + timezone.timedelta(days=32)).replace(day=1)
        month_inv = [i for i in all_inv if m <= i.issued_date < m_end]
        fin_labels.append(m.strftime("%b %y"))
        fin_billed.append(round(float(sum(i.amount for i in month_inv))))
        fin_paid.append(round(float(sum((i.amount_paid or 0) for i in month_inv))))
    finance_chart = {"labels": fin_labels, "billed": fin_billed, "paid": fin_paid}
    ...
    return render(request, "dashboard/billing.html",
                  {"invoices": invoices, "kpis": kpis, "months": months, "aging": aging,
                   "profit_rows": profit_rows[:12], "finance_chart": finance_chart})
...
def mark_invoice_paid(request, pk):
    ...
    log(request.user, "A encaissé une facture", f"{inv.client.code} · {inv.period}")
...
def record_payment(request, pk):
    ...
    inv = get_object_or_404(Invoice, pk=pk)
    try:
        amt = Decimal(str(request.POST.get("amount", "0")).replace(",", "."))
    except (InvalidOperation, TypeError):
        amt = Decimal("0")
    if amt <= 0:
        messages.error(request, "Montant invalide.")
    else:
        amt = min(amt, inv.remaining)
        inv.apply_payment(amt)
        log(request.user, "A enregistré un paiement", f"{inv.client.code} · {amt} $")
        left = inv.remaining
        messages.success(request, f"Paiement de {amt:.2f} $ enregistré — {inv.client.code}." +
                         (f" Reste {left:.2f} $." if left > 0 else " Soldée."))
    return redirect(request.META.get("HTTP_REFERER") or "dashboard:billing")
...
def copilot_ask(request):
    ...
    try:
        _cbody = json.loads(request.body or "{}")
    except ValueError:
        _cbody = {}
    q_raw = (_cbody.get("q") or "").strip()
    q = q_raw.lower()
    history = _cbody.get("history") or []
    now = timezone.localtime()
    day = now.replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    def answer():
        ...
        if m:
            ...
            if cg:
                ...
                if certs:
                    out.append(f" {certs} certification(s) expirée(s).")
                return "\n".join(out)
        ...
        # --- Local skills (no external API): maths, help, agency knowledge ---
        import re as _re
        # calculator: pure arithmetic expression
        expr = q.replace("combien font", "").replace("calcule", "").replace("=", "").strip(" ?")
        if _re.fullmatch(r"[0-9\.\,\+\-\*/x×÷%\(\)\s]+", expr) and _re.search(r"[0-9]", expr) and _re.search(r"[\+\-\*/x×÷%]", expr):
            try:
                safe = expr.replace("x", "*").replace("×", "*").replace("÷", "/").replace(",", ".").replace("%", "/100")
                val = eval(safe, {"__builtins__": {}}, {})
                val = round(val, 4)
                return f"{expr.strip()} = {val:g}"
            except Exception:
                pass
        # help / what can you do
        if any(w in q for w in ("comment ça marche", "comment sa marche", "que peux-tu", "que peux tu",
                                "aide", "aidez", "help", "à quoi tu sers", "capacités")):
            return ("Je suis votre copilote d'agence. Je réponds à partir de vos données en direct — par exemple : "
                    "« qui est en retard ? », « visites non couvertes », « revenus du mois », « clients à risque », "
                    "« soignant #007 », « état du jour », « prévisions », « heures à approuver ». "
                    "Je fais aussi les calculs (ex. « 24 × 8 »). Cliquez une suggestion ou tapez votre question.")
        # graceful, non-dismissive fallback
        return ("Je réponds surtout aux questions sur votre agence et je fais les calculs. "
                "Essayez : « qui est en retard ? », « visites non couvertes », « revenus du mois », "
                "« clients à risque », « soignant #007 », « état du jour » — ou un calcul comme « 26 × 8 ».")

    # LLM-first: a full conversational assistant that answers ANY question (operational
    # or general), grounded on live agency data. Falls back to rules if no API key.
    try:
        from ai import llm
        if llm.available():
            from incidents.models import Incident
            b = daily_brief()
            paid = sum((i.amount_paid or 0) for i in Invoice.objects.all())
            due = sum(i.remaining for i in Invoice.objects.all())
            ctx = (f"Aujourd'hui : {today.exclude(status=VisitStatus.CANCELLED).count()} visites, "
                   f"{b['completed']} terminées, {b['late']} en retard, {b['uncovered']} non couvertes. "
                   f"Actifs : {Client.objects.filter(active=True).count()} clients, "
                   f"{CG.objects.filter(active=True).count()} soignants. "
                   f"Facturation : {paid:.0f} $ encaissés, {due:.0f} $ à encaisser. "
                   f"Incidents ouverts : {Incident.objects.exclude(status='RESOLVED').count()}.")
            facts = _copilot_entity_facts(q)
            system = (
                "Tu es le Copilote IA de Panther Home Care. Tu es un assistant IA COMPLET et polyvalent : "
                "tu réponds à N'IMPORTE QUELLE question, sans aucune limite de sujet — culture générale, science, "
                "histoire, mathématiques, traduction, rédaction (e-mails, messages, textes), programmation, conseils, "
                "explications, idées créatives, etc. — exactement comme un grand assistant IA généraliste. "
                "EN PLUS, tu connais l'agence Panther et tu peux répondre aux questions opérationnelles en t'appuyant "
                "sur le CONTEXTE et les FAITS ci-dessous (visites, clients, soignants, factures, incidents). "
                "Ne refuse jamais une question générale sous prétexte qu'elle ne concerne pas Panther : réponds-y normalement. "
                "Réponds dans la langue de la question (français par défaut), de façon claire, complète et utile. "
                "Seule limite : pour une décision clinique/médicale précise sur un patient, recommande une revue par un "
                "professionnel de santé. N'invente jamais de chiffres sur l'agence ; si une donnée interne manque, dis-le.\n\n"
                "CONTEXTE AGENCE : " + ctx + ("\n\nFAITS : " + facts if facts else ""))
            msgs = []
            for m in history[-8:]:
            ...
            msgs.append({"role": "user", "content": q_raw or q})
            out = llm.chat(msgs, system=system, max_tokens=700)
            if out and out.strip():
                return JsonResponse({"answer": out.strip()})
    except Exception:
        pass
    return JsonResponse({"answer": answer()})


def _copilot_entity_facts(q):
    """Resolve a specific client/caregiver mentioned in the question into grounded facts."""
    import re
    facts = []
    m = re.search(r"client\s*#?\s*(\d+)", q)
    if m:
        c = Client.objects.filter(code=f"Client #{int(m.group(1)):03d}").first()
        if c:
            nxt = c.visits.filter(scheduled_start__gte=timezone.now()).order_by("scheduled_start").first()
            facts.append(f"{c.code} {c.full_name}, {c.care_type or 'soins'}, risque {c.risk_level.lower()}, "
                         f"{c.visits.filter(status=VisitStatus.COMPLETED).count()} visites réalisées"
                         + (f", prochaine visite {timezone.localtime(nxt.scheduled_start):%d/%m %H:%M}" if nxt else ""))
    m = re.search(r"soignant\s*#?\s*(\d+)", q)
    if m:
        from caregivers.models import Caregiver as _CG
        cg = _CG.objects.filter(code=f"Soignant #{int(m.group(1)):03d}").first()
        if cg:
            facts.append(f"{cg.code} {cg.full_name}, ponctualité {cg.punctuality_score}%")
    return " | ".join(facts)
...
def clinical(request):
    ...
    from accounts.access import can
    if not _office_only(request) or not can(request.user, "clinical"):
        return redirect("dashboard:home")
    if request.method == "POST":
        try:
            client = Client.objects.get(pk=request.POST.get("client"))
...
def convert_lead(request, pk):
    """Prospect → Client, carrying the FULL history. GET shows the preview page;
    POST performs the conversion. Never recreate the person by hand again."""
    ...
    if lead.converted_client:
        ...
        return redirect("dashboard:client_360", pk=lead.converted_client_id)
    if request.method != "POST":
        return redirect("dashboard:lead_detail", pk=lead.id)
    nums = []
    ...
    note_bits = []
    if lead.get_source_display() if hasattr(lead, "get_source_display") else lead.source:
        note_bits.append(f"Source : {lead.get_source_display() if hasattr(lead,'get_source_display') else lead.source}")
    if lead.referred_by:
        note_bits.append(f"Recommandé par : {lead.referred_by}")
    if lead.email:
        note_bits.append(f"E-mail : {lead.email}")
    if lead.notes:
        note_bits.append(lead.notes)
    client = Client.objects.create(
        code=f"Client #{(max(nums) + 1) if nums else 1:03d}",
        first_name=parts[0], last_name=parts[1] if len(parts) > 1 else "",
        care_type=lead.care_need or "Soins à domicile", active=True,
        notes=("Converti depuis prospect. " + " · ".join(note_bits)) if note_bits else "Converti depuis prospect.")
    if lead.phone:
        client.contacts.create(name=lead.name, phone=lead.phone, relationship="Prospect (contact initial)", is_primary=True)
    lead.stage = Lead.Stage.CONVERTED
    lead.converted_client = client
    lead.save(update_fields=["stage", "converted_client"])
    LeadEvent.objects.create(lead=lead, kind="STAGE", actor=request.user,
                             note=f"Converti en client {client.code} (historique conservé)")
    log(request.user, "A converti un prospect en client", client.code)
    messages.success(request, f"{lead.name} est désormais {client.code} — historique conservé.")
    return redirect("dashboard:client_360", pk=client.id)


@login_required
def lead_detail(request, pk):
    """Prospect 360 — data, contact history, referral, next action, convert preview."""
    from clients.models import Lead
    if not _office_only(request):
        return redirect("dashboard:home")
    lead = get_object_or_404(Lead, pk=pk)
    order = [k for k, _ in Lead.Stage.choices]
    idx = order.index(lead.stage) if lead.stage in order else 0
    steps = [{"key": k, "label": dict(Lead.Stage.choices)[k],
              "done": order.index(k) < idx, "current": k == lead.stage} for k in order]
    preview = {
        "carried": [b for b in [
            ("Nom", lead.name), ("Téléphone", lead.phone), ("E-mail", lead.email),
            ("Besoin de soins", lead.care_need),
            ("Source", (lead.get_source_display() if hasattr(lead, "get_source_display") else lead.source)),
            ("Recommandé par", lead.referred_by)] if b[1]],
        "missing": [m for m, ok in [
            ("Pièce d'identité", False), ("Consentement de soins", False), ("Adresse", bool(getattr(lead, "address", "")))]
            if not ok],
    }
    from ai.prospect_intel import prospect_intelligence
    return render(request, "dashboard/lead_detail.html", {
        "lead": lead, "steps": steps, "events": lead.events.select_related("actor"),
        "preview": preview, "intel": prospect_intelligence(lead)})


@login_required
@require_POST
def lead_contact(request, pk):
    """Log a contact (call/message/email/meeting/note) + optional next action / stage."""
    from clients.models import Lead, LeadEvent
    if not _office_only(request):
        return redirect("dashboard:home")
    lead = get_object_or_404(Lead, pk=pk)
    ...
    LeadEvent.objects.create(lead=lead, kind=kind, note=note, actor=request.user)
    na = request.POST.get("next_action")
    if na is not None:
        lead.next_action = na.strip()
    stage = request.POST.get("stage")
    if stage and stage in dict(Lead.Stage.choices):
        lead.stage = stage
        LeadEvent.objects.create(lead=lead, kind="STAGE", actor=request.user,
                                 note=f"Étape : {dict(Lead.Stage.choices)[stage]}")
    lead.save()
...
def invoice_new(request):
    ...
    if request.method == "POST":
        ...
        messages.success(request, f"Facture créée — {client.code} · {label}. Cliquez « Imprimer » pour l'ouvrir.")
        from django.urls import reverse as _rev
        return redirect(f"{_rev('dashboard:billing')}?created={inv.id}")
...
def invoice_view(request, pk):
    """In-app page that shows the invoice PDF embedded, with a Back button and download.
    Solves 'no way back' when opening a raw PDF."""
    from clients.models import Invoice
    ...
    if fam is None and not _finance_ok(request):
        return redirect("dashboard:home")
    return render(request, "dashboard/invoice_view.html", {"inv": inv})
...
@login_required
def family_privacy(request):
    """Family trust page: consent status + who accessed the loved one's record."""
    from clients.privacy import consent_status, set_consent
    client = _family_client(request)
    if client is None:
        return redirect("dashboard:home")
    if request.method == "POST":
        ...
        if purpose:
            set_consent(client, purpose, request.POST.get("action") == "grant",
                        signed_by=request.POST.get("signed_by", ""),
                        signature=request.POST.get("signature", ""))
            messages.success(request, "Vos preferences de confidentialite ont ete mises a jour.")
        return redirect("dashboard:family_privacy")
    return render(request, "dashboard/family_privacy.html", {
        "client": client, "consent_rows": consent_status(client),
        "access_log": client.access_logs.all()[:20]})


@login_required
def applicant_new(request):
    """In-app candidate application form (replaces the admin add page)."""
    from caregivers.models import Applicant, ApplicantEvent, ApplicationStage
    if not _office_only(request):
        return redirect("dashboard:home")
    if request.method == "POST":
        ...
        if not fn or not ln:
            ...
            return redirect("dashboard:applicant_new")
        ...
        ApplicantEvent.objects.create(applicant=a, stage=a.stage, actor=request.user,
                                      note="Candidature enregistrée")
        log(request.user, "A ajouté un candidat", f"{fn} {ln}")
        messages.success(request, "Candidat ajouté au pipeline.")
        return redirect("dashboard:applicant_detail", pk=a.id)
    return render(request, "dashboard/applicant_new.html", {})


@login_required
def applicant_detail(request, pk):
    """Candidate file + step-by-step recruitment flow."""
    from caregivers.models import Applicant, ApplicationStage
    if not _office_only(request):
        return redirect("dashboard:home")
    a = get_object_or_404(Applicant, pk=pk)
    order = [k for k, _ in ApplicationStage.choices]
    idx = order.index(a.stage) if a.stage in order else 0
    steps = [{"key": k, "label": dict(ApplicationStage.choices)[k],
              "done": order.index(k) < idx, "current": k == a.stage}
             for k in order]
    ...
    return render(request, "dashboard/applicant_detail.html", {
        "a": a, "steps": steps,
        "next_stage": nxt, "next_label": dict(ApplicationStage.choices).get(nxt),
        "prev_stage": prev, "prev_label": dict(ApplicationStage.choices).get(prev),
        "can_promote": a.stage in ("ASSESSMENT", "TRAINING", "CERTIFIED") and not a.promoted_to,
        "events": a.events.select_related("actor")[:30],
    })


@login_required
@require_POST
def applicant_advance(request, pk):
    """Move a candidate to a target stage, recording a note (the flow step)."""
    from caregivers.models import Applicant, ApplicantEvent, ApplicationStage
    if not _office_only(request):
        return redirect("dashboard:home")
    a = get_object_or_404(Applicant, pk=pk)
    ...
    note = (request.POST.get("note") or "").strip()
    score = request.POST.get("assessment_score")
    if target in dict(ApplicationStage.choices):
        a.stage = target
        if score:
            try:
                a.assessment_score = int(score)
            except (ValueError, TypeError):
                pass
        a.save()
        ApplicantEvent.objects.create(applicant=a, stage=target, actor=request.user,
                                      note=note or f"Passage à l'étape : {dict(ApplicationStage.choices)[target]}")
        log(request.user, "A fait avancer un candidat", f"{a.first_name} {a.last_name} → {target}")
        messages.success(request, "Étape mise à jour.")
    return redirect("dashboard:applicant_detail", pk=a.id)


@login_required
def family_assistant(request):
    """Assistant Panther IA — famille."""
    client = _family_client(request)
    if client is None:
        return redirect("dashboard:home")
    return render(request, "dashboard/family_assistant.html", {"client": client})


@login_required
@require_POST
def family_assistant_ask(request):
    import json as _json
    from ai.family_assistant import answer
    client = _family_client(request)
    if client is None:
        return JsonResponse({"detail": "Aucun proche lié."}, status=400)
    _b = _json.loads(request.body or "{}")
    q = (_b.get("question") or "").strip()
    if not q:
        return JsonResponse({"detail": "Question requise."}, status=400)
    return JsonResponse(answer(client, q, _b.get("history") or []))


@login_required
def incident_detail(request, pk):
    """Incident with status tracker + photo/video attachments."""
    from incidents.models import Incident, IncidentAttachment, IncidentEvent, IncidentStatus
    inc = get_object_or_404(Incident.objects.select_related("client", "caregiver", "assigned_to"), pk=pk)
    office = _office_only(request)
    if request.method == "POST" and "upload" in request.POST and (office or True):
        files = request.FILES.getlist("media") or ([request.FILES["file"]] if request.FILES.get("file") else [])
        n = 0
        for f in files:
            ct = (getattr(f, "content_type", "") or "").lower()
            name = f.name.lower()
            if ct.startswith("image") or name.endswith((".jpg", ".jpeg", ".png", ".gif", ".webp", ".heic")):
                kind = "PHOTO"
            elif ct.startswith("video") or name.endswith((".mp4", ".webm", ".mov", ".m4v", ".avi")):
                kind = "VIDEO"
            else:
                kind = "FILE"
            if f.size > 60 * 1024 * 1024:
                messages.error(request, f"{f.name} dépasse 60 Mo.")
                continue
            IncidentAttachment.objects.create(incident=inc, file=f, kind=kind,
                                              caption=request.POST.get("caption", "").strip(),
                                              uploaded_by=request.user)
            n += 1
        if n:
            IncidentEvent.objects.create(incident=inc, note=f"{n} pièce(s) jointe(s) ajoutée(s)", actor=request.user)
            messages.success(request, f"{n} pièce(s) ajoutée(s).")
        return redirect("dashboard:incident_detail", pk=inc.pk)
    if request.method == "POST" and request.POST.get("status") and office:
        new = request.POST["status"]
        if new in dict(IncidentStatus.choices):
            inc.status = new
            if new == "RESOLVED":
                inc.resolved_at = timezone.now()
            inc.save()
            IncidentEvent.objects.create(incident=inc, status=new, actor=request.user,
                                         note=request.POST.get("note", "").strip())
            log(request.user, "A mis à jour un incident", f"{inc.title} → {new}")
            messages.success(request, "Statut mis à jour.")
        return redirect("dashboard:incident_detail", pk=inc.pk)
    order = [k for k, _ in IncidentStatus.choices]
    idx = order.index(inc.status) if inc.status in order else 0
    steps = [{"key": k, "label": dict(IncidentStatus.choices)[k],
              "done": order.index(k) <= idx, "current": k == inc.status} for k in order]
    return render(request, "dashboard/incident_detail.html", {
        "inc": inc, "steps": steps, "office": office,
        "attachments": inc.attachments.select_related("uploaded_by"),
        "events": inc.events.select_related("actor"),
        "status_choices": IncidentStatus.choices})


@login_required
@require_POST
def client_ask(request, pk):
    """Ask Panther about ONE client — grounded on that client's timeline + facts."""
    if not _office_only(request):
        return JsonResponse({"answer": "Réservé au bureau."}, status=200)
    client = get_object_or_404(Client, pk=pk)
    ...
    from ai.timeline import client_timeline
    from ai.attention import attention_for_client
    tl = client_timeline(client, limit=25)
    lines = [f"{e['at']:%d/%m %H:%M} — {e['title']}" + (f" ({e['detail']})" if e['detail'] else "") for e in tl]
    att = attention_for_client(client)
    facts = (f"{client.code} {client.full_name}, {client.care_type or 'soins'}, risque {client.risk_level.lower()}. "
             f"Signaux d'attention: " + (", ".join(s['label'] for s in att['signals']) if att else "aucun") + ".")
    context = facts + "\n\nPARCOURS RÉCENT:\n" + "\n".join(lines[:25])
    try:
        from ai import llm
        if llm.available():
            system = ("Tu es le copilote Panther. Réponds à la question du coordinateur sur CE client, "
                      "en français, uniquement à partir du CONTEXTE (parcours + faits). Sois concis et factuel. "
                      "N'invente rien ; si l'info n'est pas dans le contexte, dis-le. Pour une décision clinique, "
                      "recommande une revue professionnelle.\n\nCONTEXTE:\n" + context)
            out = llm.chat([{"role": "user", "content": q}], system=system, max_tokens=500)
            if out and out.strip():
                return JsonResponse({"answer": out.strip()})
    except Exception:
        pass
    # deterministic fallback: summarise the recent timeline
    summary = f"Voici le parcours récent de {client.full_name} :\n" + "\n".join(f"• {l}" for l in lines[:8])
    if att:
        summary += "\n\nÀ examiner : " + ", ".join(s['label'] for s in att['signals']) + f".\nSuggestion : {att['next']}"
    return JsonResponse({"answer": summary})


@login_required
def client_edit(request, pk):
    """Admin/office edit of a client's details (correct a name, address, etc.)."""
    if not _office_only(request):
        return redirect("dashboard:home")
    client = get_object_or_404(Client, pk=pk)
    if request.method == "POST":
        ...
        client.care_type = request.POST.get("care_type", client.care_type).strip()
        client.preferred_language = request.POST.get("preferred_language", client.preferred_language).strip()
        if request.POST.get("risk_level"):
            client.risk_level = request.POST["risk_level"]
        client.active = request.POST.get("active") == "on"
        client.save()
        log(request.user, "A modifié un client", client.code)
        messages.success(request, "Fiche client mise à jour.")
        return redirect("dashboard:client_360", pk=client.id)
    return render(request, "dashboard/client_edit.html", {"client": client, "risk_choices": Client._meta.get_field("risk_level").choices})
...
def caregiver_edit(request, pk):
    ...
    cg = get_object_or_404(Caregiver, pk=pk)
    if request.method == "POST":
        cg.first_name = request.POST.get("first_name", cg.first_name).strip()
        cg.last_name = request.POST.get("last_name", cg.last_name).strip()
        cg.phone = request.POST.get("phone", cg.phone or "").strip()
        if request.POST.get("pay_rate"):
            try:
                cg.pay_rate = request.POST["pay_rate"]
            except Exception:
                pass
        cg.active = request.POST.get("active") == "on"
        cg.save()
        log(request.user, "A modifié un soignant", cg.code)
        messages.success(request, "Fiche soignant mise à jour.")
        return redirect("dashboard:caregiver_360", pk=cg.id)
    return render(request, "dashboard/caregiver_edit.html", {"cg": cg})


@login_required
def user_edit(request, pk):
    """Correct a user's name / email (e.g. a mistyped registration)."""
    from accounts.models import User
    if not (request.user.is_staff or request.user.is_superuser or getattr(request.user, "is_manager", False)):
        return redirect("dashboard:home")
    ...
    if request.method == "POST":
        ...
        log(request.user, "A modifié un utilisateur", u.username)
        messages.success(request, "Utilisateur mis à jour.")
        return redirect("dashboard:users")
    return render(request, "dashboard/user_edit.html", {"u": u})


@login_required
def coverage_gaps(request):
    """Panther Care Intelligence — high-risk coverage gaps with ranked caregivers + why."""
    if not _office_only(request):
        return redirect("dashboard:home")
    now = timezone.localtime()
    horizon = now + timezone.timedelta(days=7)
    uncovered = (Visit.objects.filter(status=VisitStatus.UNCOVERED, scheduled_start__gte=now,
                                      scheduled_start__lte=horizon)
                 .select_related("client").order_by("scheduled_start")[:12])
    gaps = []
    for v in uncovered:
        ranked = match_caregivers(v, top_n=3)
        mins = int((v.scheduled_start - now).total_seconds() // 60)
        cards = []
        for r in ranked:
            km = r.get("distance_km")
            checklist = [
                ("Compétences requises", bool(r.get("skills_ok"))),
                ("Disponible", True),
                (f"À proximité ({round(km)} km)" if km is not None else "Proximité", km is None or km <= 12),
                ("A déjà accompagné le client" if r.get("continuity_count") else "Nouveau pour ce client", bool(r.get("continuity_count"))),
                ("Pas d'heures supplémentaires", r.get("workload") != "Élevée"),
            ]
            cards.append({
                "cg": r["caregiver"], "score": r["score"], "tier": r["tier"],
                "km": round(km, 1) if km is not None else None,
                "workload": r.get("workload"), "continuity": r.get("continuity_count", 0),
                "why": r.get("why", ""), "checklist": checklist,
            })
        gaps.append({
            "visit": v, "client": v.client, "mins": mins,
            "level": "high" if mins <= 24 * 60 else "medium",
            "cards": cards,
        })
    return render(request, "dashboard/coverage.html", {"gaps": gaps})


def _command_answer(q):
    """Grounded fallback answer for the command bar (uses Claude if configured)."""
    try:
        from ai import llm
        from incidents.models import Incident
        b = daily_brief()
        today = Visit.objects.filter(scheduled_start__date=timezone.localdate())
        paid = sum((i.amount_paid or 0) for i in Invoice.objects.all())
        due = sum(i.remaining for i in Invoice.objects.all())
        ctx = (f"Aujourd'hui : {today.exclude(status=VisitStatus.CANCELLED).count()} visites, "
               f"{b['completed']} terminées, {b['late']} en retard, {b['uncovered']} non couvertes. "
               f"Clients actifs {Client.objects.filter(active=True).count()}, "
               f"soignants actifs {CG.objects.filter(active=True).count()}. "
               f"Encaissé {paid:.0f}, à encaisser {due:.0f}. "
               f"Incidents ouverts {Incident.objects.exclude(status='RESOLVED').count()}.")
        if llm.available():
            out = llm.chat([{"role": "user", "content": q}],
                           system=("Tu es l'assistant IA de Panther. Réponds à toute question (générale ou sur l'agence), "
                                   "brièvement et clairement. Pour les données de l'agence, appuie-toi sur le CONTEXTE sans "
                                   "rien inventer.\n\nCONTEXTE : " + ctx), max_tokens=300)
            if out and out.strip():
                return out.strip()
        return ("Aucune correspondance directe. Essayez : « visites non couvertes », « incidents », "
                "« factures », « clients à risque » ou un nom de client.")
    except Exception:
        return "Essayez : « visites non couvertes », « incidents », « factures » ou un nom de client."


@login_required
@require_POST
def command_bar(request):
    """Universal command bar — routes to a module, opens a record, or answers."""
    import re
    from django.urls import reverse
    from django.db.models import Q
    if not request.user.is_office:
        return JsonResponse({"type": "none"})
    q = (json.loads(request.body or "{}").get("q") or "").strip()
    if not q:
        return JsonResponse({"type": "none"})
    ql = q.lower()

    # a genuine question -> answer directly (don't navigate)
    if "?" in q or re.match(r"^(combien|quel|quels|quelle|pourquoi|comment|est-ce|qui|où|ou\b|que\b|qu')", ql):
        return JsonResponse({"type": "answer", "text": _command_answer(q)})

    # clients à risque
    if re.search(r"risque", ql) and "client" in ql:
        return JsonResponse({"type": "redirect", "url": reverse("dashboard:clients") + "?risk=HIGH", "label": "Clients à risque"})

    # open a person by name
    name = None
    if re.search(r"dossier|ouvre|ouvrir|fiche|\bclient|cliente|\bmme\b|\bmr\b|madame|monsieur", ql):
        _stop = {"le", "la", "les", "de", "du", "des", "d", "l", "dossier", "fiche", "client", "cliente",
                 "ouvre", "ouvrir", "pour", "mme", "mr", "m", "madame", "monsieur", "moi", "montre", "montre-moi"}
        name = " ".join(w for w in re.split(r"[\s']+", ql) if w and w not in _stop).strip()
    intents = [
        (r"non couvert|remplac|couvertur|coverage", "dashboard:coverage", "Visites non couvertes"),
        (r"incident", "dashboard:incidents", "Incidents"),
        (r"r[ée]union|meeting", "collab:meetings", "Réunions"),
        (r"travail|à faire|a faire|attention", "collab:worklist", "Travail à faire"),
        (r"prospect|crm|lead", "dashboard:leads", "Prospects & CRM"),
        (r"factur|paiement|impay|solde|en retard", "dashboard:billing", "Facturation"),
        (r"planning|planif|agenda|visites? aujour", "dashboard:schedule", "Planning"),
        (r"conformit|certif", "dashboard:compliance", "Conformité"),
        (r"carte|opérations|map", "dashboard:map", "Carte des opérations"),
        (r"temps r[ée]el|live", "dashboard:live", "Temps réel"),
        (r"soignant|caregiver|personnel", "dashboard:caregivers", "Soignants"),
    ]
    if not name and len(ql.split()) <= 3 and not any(re.search(pt, ql) for pt, _, _ in intents) and "?" not in q:
        name = ql
    if name:
        cs = list(Client.objects.filter(Q(first_name__icontains=name) | Q(last_name__icontains=name) | Q(code__icontains=name))[:6])
        if len(cs) == 1:
            return JsonResponse({"type": "redirect", "url": reverse("dashboard:client_360", args=[cs[0].id]), "label": cs[0].full_name})
        if cs:
            return JsonResponse({"type": "results", "items": [
                {"label": f"{c.code} — {c.full_name}", "url": reverse("dashboard:client_360", args=[c.id])} for c in cs]})
    for pat, urlname, label in intents:
        if re.search(pat, ql):
            try:
                return JsonResponse({"type": "redirect", "url": reverse(urlname), "label": label})
            except Exception:
                pass
    return JsonResponse({"type": "answer", "text": _command_answer(q)})
...
def incident_new(request):
    """Report an accident/incident with photo/video upload, in one step."""
    from incidents.models import Incident, IncidentAttachment, IncidentEvent, IncidentStatus
    from caregivers.models import Caregiver
    if not (_office_only(request) or getattr(request.user, "caregiver_profile", None)):
        return redirect("dashboard:home")
    CATS = ["Chute", "Blessure", "Médication", "Comportement", "Objet manquant", "Malaise", "Autre"]
    if request.method == "POST":
        cat = (request.POST.get("category") or "").strip()
        desc = (request.POST.get("description") or "").strip()
        title = (request.POST.get("title") or "").strip() or (cat or "Incident signalé")
        if cat and cat.lower() not in title.lower():
            title = f"{cat} — {title}" if title != cat else cat
        sev = request.POST.get("severity") if request.POST.get("severity") in dict(Incident._meta.get_field("severity").choices) else "MEDIUM"
        client_id = request.POST.get("client") or None
        cg = getattr(request.user, "caregiver_profile", None)
        cg_id = request.POST.get("caregiver") or (cg.id if cg else None)
        inc = Incident.objects.create(
            title=title, description=desc, severity=sev,
            status=IncidentStatus.OPEN if hasattr(IncidentStatus, "OPEN") else "OPEN",
            client_id=client_id if client_id else None,
            caregiver_id=cg_id if cg_id else None)
        # media
        n = 0
        for f in request.FILES.getlist("media"):
            ct = (getattr(f, "content_type", "") or "").lower(); name = f.name.lower()
            if ct.startswith("image") or name.endswith((".jpg", ".jpeg", ".png", ".gif", ".webp", ".heic")):
                kind = "PHOTO"
            elif ct.startswith("video") or name.endswith((".mp4", ".webm", ".mov", ".m4v", ".avi")):
                kind = "VIDEO"
                ...
                kind = "FILE"
            if f.size <= 60 * 1024 * 1024:
                IncidentAttachment.objects.create(incident=inc, file=f, kind=kind, uploaded_by=request.user)
                n += 1
        IncidentEvent.objects.create(incident=inc, status=inc.status, actor=request.user,
                                     note=f"Incident déclaré{f' avec {n} pièce(s) jointe(s)' if n else ''}")
        log(request.user, "A déclaré un incident", inc.title)
        messages.success(request, "Incident déclaré. Le coordinateur est notifié.")
        return redirect("dashboard:incident_detail", pk=inc.id)
    return render(request, "dashboard/incident_new.html", {
        "cats": CATS, "clients": Client.objects.filter(active=True).order_by("code"),
        "severities": Incident._meta.get_field("severity").choices,
        "is_caregiver": bool(getattr(request.user, "caregiver_profile", None))})


def _msg_role(user):
    """finance -> only finance channel; advisor/admin/coordinator -> all."""
    r = getattr(user, "role", "")
    if r == "FINANCE":
        return "FINANCE"
    return "ALL"
...
@login_required
def messages_center(request, pk=None, topic="CARE"):
    """Agency communication centre — two panes: conversations list + open thread."""
    from clients.models import Message, Client
    if not _office_only(request):
        return redirect("dashboard:home")
    scope = _msg_role(request.user)
    base = Message.objects.all()
    if scope == "FINANCE":
        base = base.filter(topic="FINANCE")
    # conversations list (latest per client+channel)
    seen, threads = set(), []
    for m in base.select_related("client").order_by("-created_at"):
        key = (m.client_id, m.topic)
        if key in seen:
            continue
        seen.add(key)
        unread = Message.objects.filter(client_id=m.client_id, topic=m.topic,
                                        from_agency=False, read=False).count()
        threads.append({"client": m.client, "topic": m.topic,
                        "topic_label": dict(Message.TOPICS).get(m.topic, m.topic),
                        "last": (m.body or "")[:60], "at": m.created_at, "unread": unread,
                        "active": (pk and m.client_id == int(pk) and m.topic == topic)})
    threads.sort(key=lambda t: (0 if t["unread"] else 1, -t["at"].timestamp()))

    selected, thread = None, []
    if pk:
        if topic not in ("CARE", "FINANCE"):
            topic = "CARE"
        if scope == "FINANCE" and topic != "FINANCE":
            return redirect("dashboard:messages_inbox")
        client = get_object_or_404(Client, pk=pk)
        if request.method == "POST":
            body = (request.POST.get("body") or "").strip()
            if body:
                Message.objects.create(client=client, sender=request.user, from_agency=True, topic=topic, body=body)
                try:
                    if client.family_user:
                        from notifications.services import notify
                        notify(client.family_user, f"Message de l'agence — {dict(Message.TOPICS).get(topic)}", body[:120])
                except Exception:
                    pass
            return redirect("dashboard:client_conversation", pk=client.id, topic=topic)
        Message.objects.filter(client=client, topic=topic, from_agency=False, read=False).update(read=True)
        thread = Message.objects.filter(client=client, topic=topic).select_related("sender")
        selected = {"client": client, "topic": topic, "topic_label": dict(Message.TOPICS).get(topic)}
    return render(request, "dashboard/messages_center.html",
                  {"threads": threads, "scope": scope, "selected": selected, "thread": thread})


@login_required
def access_matrix(request):
    """Admin-editable role → module permission matrix."""
    from accounts.models import AccessMatrix, User, Role
    from accounts.access import AREAS, ROLES, DEFAULTS, AREA_KEYS
    is_admin = request.user.is_superuser or getattr(request.user, "role", "") in ("ADMIN", "MANAGER")
    if not is_admin:
        return redirect("dashboard:home")
    mx = AccessMatrix.get()
    if request.method == "POST":
        data = {}
        for role, _ in ROLES:
            data[role] = {area: (request.POST.get(f"{role}:{area}") == "on") for area in AREA_KEYS}
        mx.data = data
        mx.save()
        log(request.user, "A modifié la matrice d'accès", "")
        messages.success(request, "Permissions mises à jour.")
        return redirect("dashboard:access_matrix")
    # current effective values
    grid = {}
    for role, _ in ROLES:
        rd = (mx.data or {}).get(role)
        grid[role] = {a: (rd.get(a) if isinstance(rd, dict) and a in rd else (a in DEFAULTS.get(role, set())))
                      for a in AREA_KEYS}
    return render(request, "dashboard/access_matrix.html", {"areas": AREAS, "roles": ROLES, "grid": grid})
